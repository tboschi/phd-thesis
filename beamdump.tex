...
The configuration of the ND system in the hall is shown in \reffig{fig:duneND}.
...
and then decay forming a neutrino beam (see \refsec{sec:nu_acc}).
...
A summary of the detector features relevant for this analysis is reported in~\reftab{tab:nd}, %
...
and the most relevant for this study, as it will be explained in~\refsec{sec:numevt}.
...
the muonic and electronic ones which are enhanced, as explained in \refsec{sec:production}.
...
With the help of a purpose-built Monte Carlo simulation, the $D_s^+$ four-momenta are generated starting from~\refeq{eq:dsflux} %
...
of the order of \np{e-13}\,s, the meson path is not affected by the horn system nor by interactions with other accelerator components. 
...
as illustrated in~\reffig{fig:taudist} in which the enhancement discussed in \refsec{sec:production} is not included.
...
The channels which open up for masses above the kaon mass are more challenging to deal with, from an experimental point of view.
...
Cosmogenic background is also not considered here, even though a rate of \np{2.7}\,Hz/m\tapi{2} cosmic rays %
...
\end{equation}
and the signal efficiency $W_d(E)$ is introduced in \refsec{sec:numevt}.
%%BETTER!!!!!!

As an example of the selection process, the results of the background analysis for an HNL with mass $m_N = 450$\,MeV %
is reported here.
...
The last row of each table shows the integrated efficiency of the selection cuts, introduced in \refeq{eq:integrated_eff}.
...
The background rejection and signal efficiency are shown in \reftab{tab:eff_two}.
...
The background rejection and signal efficiency are shown in \reftab{tab:eff_three}.
%\enlargethispage{\baselineskip}
...
The background rejection and signal efficiency are shown in \reftab{tab:eff_EM}.
...
\section{Sensitivities of the DUNE ND}
\label{sec:results}
...
\refsec{sec:dominant} shows the constraint that the DUNE ND can place on a simplified scenario %
...
In \reftab{tab:senseMix}, the sensitivity results for two dominant mixings are reported.
...
They all give strong sensitivities, especially for masses below 0.5\,GeV as it can be seen in \refsec{sec:results}.
...
\begin{figure}
	\centering
	\resizebox{0.49\linewidth}{!}{\input{pics/DUNE_HNL_E_real.tex}}
	\resizebox{0.49\linewidth}{!}{\input{pics/DUNE_HNL_M_real.tex}}

	\bigskip
	\resizebox{0.49\linewidth}{!}{\input{pics/DUNE_HNL_T_real.tex}}
	%
...
Moreover, a different ND configuration was assumed and no background study was performed in \refref{Krasnov:2019kdc}.

Finally, in \reffig{fig:sensAll} the overall sensitivity is compared with respect to regions allowed by neutrino mass models.
...
Correct charge identification is therefore a necessary requirement to distinguish between %
...
In this chapter, the sensitivity of the DUNE ND to heavy neutral leptons in beam-dump-style searches has been estimated.
The analysis is based on the calculations presented in \refcha{cha:mass_models}, %
...
reaching \np{e-10} especially with two-body semi-leptonic channels via $|U_{e N}|^2$ and $|U_{\mu N}|^2$.
