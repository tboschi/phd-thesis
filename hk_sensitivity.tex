...
The test statistics of this analysis is defined as a ``pull approach'' $\chi^2$~\cite{Fogli:2002pt}, %
starting from a Poissonian likelihood
\begin{equation}
...
which are built with oscillation parameters $\vartheta$.
and the product runs over the total number of bins.
The $\chi^2$ is the following log-likelihood ratio
\begin{equation}
	\label{eq:log_ratio}
	\chi^2(\vartheta) = -2 \log \qty[\frac{\mathfrak{L}(E_n, O_n)}{\mathfrak{L}(E_n, O_n)}] =
		2 \sum_n \qty[E_n - O_n + O_n \log \qty(\frac{E_n}{O_n})]\ ,
\end{equation}
to which ``pull'' parameters $\uj{\varepsilon}$ are introduced to account for systematic uncertainties, %
by replacing
\begin{equation}
	\label{eq:pull}
	E_n\ \longrightarrow\ E_n\,\qty(1 + \textstyle\sum_j f_j^n \varepsilon_j)\ 
\end{equation}
and adding the variances and covariances of such parameters.
...
However, being $\rho$ the correlation matrix, the $\varepsilon$ are promoted to denote the systematic uncertainties, %
...
Once defined the observed sample, the $\chi^2$ is then computed at each point of the oscillation parameter space %
by profiling it with respect to the pulls $\varepsilon$:
\begin{equation}
	\label{eq:chi2_min}
