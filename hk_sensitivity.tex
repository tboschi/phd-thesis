...
\begin{align}
	\psi(x) &\longmapsto \psi^C(x) = \xi_C\, \mathcal{C}\,\cj{\psi}^T (x) \\
	\psi(x) &\longmapsto \cj{\psi^C}(x) = - \xi_C^*\,\psi^T (x)\,\mathcal{C}^\dagger\ ,
\end{align}
where $\xi_C$ is a unitary phase and $\mathcal{C}$ is the charge conjugation operator.
The parity transformation, instead, inverts spatial coordinates and acts on spinor fields as
\begin{align}
	\psi(x) &\longmapsto \psi^P(x') = \xi_P\, \gamma^0 \,\psi(x) \\
	\psi(x) &\longmapsto \cj{\psi^P}(x') = \xi_P^*\,\cj{\psi}(x)\,\gamma^0\ ,
\end{align}
where $\gamma^0$ is on of the Dirac matrices.
...
Vector currents $\cj{\psi}_a \gamma^\mu \psi_b$ and axial currents $\cj{\psi}_a \gamma^\mu \gamma^5 \psi_b$ %
transforms oppositely under charge conjugation and spatial inversion, and therefore the $V-A$ structure %
...
\begin{figure}
	\centering
	\includegraphics[width=0.6\linewidth]{pics/HKDRcpSens.png}
	\caption{Expected significance to exclude CP conservation, %
		in case of normal hierarchy, where the mass hierarchy is assumed to %
		be known~\cite{Abe:2018uyc}.}
	\label{fig:hkdr}
\end{figure}
