...
The main channel of interest for long baseline experiments (LBL) is the appeareance channel $\nu_\mu \to \nu_e$.
...
The amount of change is therefore parametrised by the $\varepsilon_j$ in units of the uncertainty $\sigma_j$.
...
A ``delayed-gratification scheme'' is adopted for $\lambda$~\cite{Transtrum2012}, using an inital value of $\lambda^{(0)} = 1$ and finding %
...
\begin{figure}
	...
	\caption{The top panels of each figure show the $\chi^2$ profile for %
		...
		The degenracy in the $\chi^2$ for $\Delta m_{32}^2$ is slightly with anticorrelated systematics, %
		...
		the curve computed wihtout systematic uncertainties (black).
...
The effect is simulated by considering only partial statistics and so using rescaled event sampels.
With only a quarted of the data collected, a maximally violating CP phase could be excluded at $3\sigma$, %
...
For example, if the true mass ordering was normal hierarchy and interved hierarchy is assumed in the fit, %
the sensitivity for $\delta_\text{CP} < 0$ remains unaffacted, whereas the one for $\delta_\text{CP} > 0$ worsens.
...
The $\chi^2$ as a function of $\delta_\text{CP}$, in \reffig{fig:0_11a_11b_8_dCP_S13} changes slightly with a diffeerent flux uncertainty, %
but it is not very much affected by a variation of the SK energy scale.
This is true also the $\theta_{13}$, the $\chi^2$ profile of which is also in \reffig{fig:0_11a_11b_8_dCP_S13}.
As it can be seen in \reffig{fig:0_11a_11b_8_M23_S23}, the outcome is opposite for %
$\Delta m_{32}^2$ and $\theta_{23}$, which show a larger deviation from the nominal %
model away from the best fit value.
The contour plot of \reffig{fig:0_11a_11b_8_contour} confirms that no appreciable deviations from the nominal model %
