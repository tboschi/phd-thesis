...
of weak interactions (see \refeq{eq:currents_ccnc}) violates C and P individually.
...
The probability of flavour oscillation in vacuum is given in \refeq{eq:oscillation_probability}.
...
Starting from the unitarity of the mixing matrix, for example $U\,U^\dagger = 1$, we have
\begin{equation}
...
The neutrino beam is generated by a 30\,GeV proton beam in the way described in \refsec{sec:nu_atm_acc}.
...
The simulation of the HK detector heavily relies on the techniques established with the current experiments.
...
shown respectively in \reffig{fig:nuenorm_dCP}, \ref{fig:nuenorm_M23}, \ref{fig:nuenorm_S13}, and \ref{fig:nuenorm_S23} %
...
In \reffig{fig:nuenorm_S23_M23}, the contour levels of the $\chi^2$ versus $\Delta m_{32}^2$ and $\sin^2 \theta_{23}$ %
...
The importance of this study set can also be appreciated in \reffig{fig:nuenorm_sensitivity}.
...
If the mass hierarchy is unknown, the sensitivity can be recovered by including the atmospheric sample.
