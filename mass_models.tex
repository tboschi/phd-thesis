...
The cartoon of \reffig{fig:iss_spectrum} shows how the mass spectrum changes with LNV terms, %
highlighting how the mass degeneracy in the LNC limit is resolved when LNV contributions are added.
The notation ``ISS\,$(a,b)$'' will be used to denote a realisatione of the model with $a$ and $b$ new gauge singlets of, %
...
\begin{figure}
	...
	\begin{fmffile}{twobody}
		\begin{fmfgraph*}(100,70)
			...
			\fmf{fermion}{v,l}
			\fmf{photon,tension=2.}{v,w}
			\fmf{double,tension=2.}{w,p}
			\fmflabel{$N$}{N}
			\fmfblob{0.1w}{w}
			\fmfv{label=$P^+$,label.angle=0}{p}
			\fmfv{label=$\ell^-$,label.angle=0}{l}
			%\fmflabel{$P^+$}{p}
			%\fmflabel{$\ell^-$}{l}
		\end{fmfgraph*}
	\end{fmffile}
	...
	%\hfill
	%\begin{fmffile}{HNLdecay}
	%	\begin{fmfgraph*}(220,80)
	%		\fmfstraight
	...
	%		\fmfright{n2,l1,l2,N}
	%		\fmftop{n5,l0,n3,n4}
	%		\fmf{double,tension=3.0}{P,vP}
	%		\fmf{photon,tension=3.0}{vP,vN}
	%		\fmf{fermion,tension=0.5}{l0,vN}
	%		\fmf{fermion,tension=0.5,label=$N$}{vN,vI}
	%		\fmf{fermion,tension=0.5}{vI,N}
	%		\fmf{photon,tension=1.5, left=0}{vI,vO}
	%		\fmf{fermion}{l2,vO,l1}
	%		\fmfblob{0.05w}{vP}
	%		\fmflabel{$K^+$}{P}
	%		\fmflabel{$\mu^+$}{l0}
	%		\fmflabel{$e^-$}{N}
	...
	%		\fmflabel{$\mu^+$}{l2}
	%		%\fmfblob{0.15w}{vl}
	%		%\fmfv{decoration.shape=cross,decor.size=0.10w,decor.angle=45}{x}
	%		%\fmf{phantom,tension=0.7}{o2,vr,o1}
	%		%\fmffreeze
	%		%\fmf{fermion}{x,vr,o1}
	%		%\fmf{fermion,fore=red}{x,o2}
	%		%\fmfv{label=$U_{\mu 4}^*$,label.angle=30}{x}
	%		%\fmfv{label=$\mu^+$,label.angle=0}{o1}
	%		%\fmfv{label=\textcolor{red}{$N$},label.angle=0}{o2}
	%	\end{fmfgraph*}
	%\end{fmffile}
...
\end{equation}
introduced in~\refref{Atre:2009rg}, where $x, y$ and squared mass ratios.
After integrating over the angular variables, it is found that %
both the pseudoscalar meson decay rates do not depend on helicity, as expected,
%
%	\begin{align*}
...
\end{equation}
The charged pseudo-vector decay rates agree with \refrefs{Bondarenko:2018ptm, Atre:2009rg, Helo:2010cw, Gorbunov:2007ak} %
...
Other interesting but neglected channels are the multipion decay modes discussed in \refref{Bondarenko:2018ptm}, %
...
Their behaviour is plotted as a function of the heavy neutrino mass on the right panel of \reffig{fig:scale}.
