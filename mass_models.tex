...
and necessarily implies that the neutrino mass splittings are non zero, as seen in \refsec{sec:neutrino_vacuum}.
...
As done in \refsec{sec:neutrino_mixing}, a simple means of doing so is to introduce the right-handed counterpart of SM neutrinos,
...
In this chapter, low-scale seesaw models are dicussed, linking the origin of light neutrinos to %
a new phenomenologically viable phsysics scale in \refsec{sec:model}.
Formulae to compure polarised decay rates and production rates  %
are presented in respectively \refsec{sec:decay} and \refsec{sec:production}.
The correct implementation of the differential widths is an important step to develop a reliable %
study of searches for HNL in future experiments (see \refcha{cha:hnl_dune}).

\section{Heavy neutrinos in seesaw models}
\label{sec:model}

With $n$ new SM singlets $\mathcal{N}_i$, the extendend Lagrangian becomes
\begin{equation}
	\label{eq:newmodel}
...
\begin{equation}
	P_L\,\bs{\nu} \equiv (\nu_e, \nu_\mu, \nu_\tau, N_1, ..., N_n)\ ,
\end{equation}
and $C$ denotes a charge-conjugation (see \refcha{cha:cp_hk}).
...
\end{equation}
The top-left zero in \refeq{eq:seesaw_mass_matrix} arises as a consequence of $\text{SU(2)}_L$ symmetry, %
...
Another way to avoid the bound of \refeq{eq:TypeI_mixing_bound} in presence of more than three sterile neutrinos %
is by the cancellation between their contributions to neutrino masses.

This is achieved in a class of models with apporixmate lepton number (LN), %
...
Following \refeq{eq:newmodel} and \refeq{eq:typeI}, the most general mass matrices are given when
\begin{equation}
...
with small masses dictated by the perturbation.
...
%More generically, let's use ISS\,$(a,b)$ to denote the model with $a$ and $b$ new gauge singlets of, %
...
lepton number violating effects will be disreagarded and therefore the distinction between pseudo-Dirac %
...
In this section, the heavy neutrino decay rates and polarised distributions necessary for studies in experimental searches are presented.
...
The semi-leptonic meson decays are some of the most important channels identified in previous %
...
\end{equation}
introduced in~\refref{Atre:2009rg}.
...
(see \refeq{eq:kappa_factors}).
The charged pseudo-vector decay rates agrees with \refrefs{Bondarenko:2018ptm, Atre:2009rg, Helo:2010cw, Gorbunov:2007ak} %
...
Let's assign the momenta to the particles in the three-body decay as follows
\begin{equation}
...
\end{align*}
It is not surprising in view of \refeq{eq:gamma_NC_sum} to observe that these coefficient are related %
...
that generate light neutrinos (see \refsec{sec:nu_atm_acc}).
If kinematically allowed, heavy neutrino states can be sourced from these decays of mesons and charged leptons.
The neutrino production channels considered in this analysis are shown in \reftab{tab:branch}, %
...
\begin{table}[t]
	...
	\caption{Production channels at beam dump facilities yielding neutrinos, with the respective branching %
		ratios (taken from \refref{Tanabashi:2018oca}).
...
\begin{figure}
	...
	\caption{On the left, the factors $\mathcal{S}^-$ (solid) and $\mathcal{S}^+$ (dashed) %
		...
		When the mass approeaches to zero, the percentage of neutrinos procued %
