...
\end{equation}
The angle $\vartheta_\text{W}$ is known as Weinberg angle and it is such that $\sin^2\vartheta_\text{W} = 0.2$.
...
Therefore, neutrino states can be arbitrarily choosen such that $\hat{\bs{\nu}}_L = V^{\ell\dagger}_L\, \bs{\nu}_L$, %
...
It follows that the probability of producing and detecting a neutrino in the same point of space-time is, unsuprsisingly, one.
...
It could happen that neutrinos are produced or detected incoherently, for which intereference terms %
...
Finally, if the density is much higher than the reonance one, i.e.\  $A_\text{CC} \gg \Delta m^2 \cos\theta$, %
...
because the neutrinoless manifestation of this decay is a true lepton number violating process, %
...
\begin{table}
	\small
	\centering
	\captionof{table}{Processes occurring in the $pp$ chain (left) and CNO cycles (right) that emits $\nu_e$.
...
\begin{figure}
	\begin{minipage}[t]{0.4\textwidth}
		...
		\captionof{figure}{Measurement of cosmic rays over the entire energy spectrum, %
			where the change of energy dependence is visible (\emph{knee})~\cite{Anchordoqui:2002hs}.}
		\label{fig:CR_flux}
	\end{minipage}
	\hfill
...
also thanks to the implementation of geomagnetic models~\cite{Honda:2004yz, Honda:2006qj}.
...
for practical reasons this contribution is disregarded.
...
To improve the quality of the neutrino beam, a focusing system is typically put into place %
...
By changing the direction of the current in the horns it is possible to select particles with opposite sign.
...
as pions and muons, and therefore the neutrino beam presents an instrinsic spread.
...
neutrino energy is fixed and known, as shown in \reffig{fig:t2k_flux}.
...
In the following sections the most important neutrino interactions with matter are reviewd, %
