...
and has proven that the neutrinos are not all massless as imposed by the SM.
Mass terms for the neutrinos can be actually included in the SM, although with the implication of theoretical and naturalness problems.
...
\end{equation}
where $\varepsilon_{a b c}$ is the Levi-Civita tensor.

\subsection{Electroweak interactions}
\label{sec:ew_interactions}

Expanding the covariant derivative and ignoring kinetic terms, the interaction terms for the lepton sector are retrieved
\begin{equation}
...
\end{equation}
where $\alpha$ is the family generation index.
Defining the combinations
\begin{align}
	\label{eq:fields}
	W^\mu &= \flatfrac{\qty(A_1^\mu - i A_2^\mu)}{\sqrt{2}} \\
	%W^{\mu\dagger} &= \flatfrac{\qty(A_1^\mu + i A_2^\mu)}{\sqrt{2}} \\	?? redundant
	%W^{\mu(\dagger)} &= \flatfrac{\qty(A_1^\mu \mp i A_2^\mu)}{\sqrt{2}} \\
...
the new field $Z^\mu$ also mediates neutral current processes.
...
\end{align}
The constant $g'$ has been rewritten in terms of $g$ and $\vartheta_\text{W}$ by setting to zero the coupling of neutrinos %
to the electromagnetic field which implies
\begin{equation}
	g\, \sin \vartheta_\text{W} = g'\, \cos \vartheta_\text{W}\ .
\end{equation}
The weak mixing angle $\vartheta_\text{W}$ is also known as \emph{Weinberg angle} and it is estimated %
to be approximately $\sin^2\vartheta_\text{W} \simeq 0.231$ from either studies of neutrino neutral-current processes at low energies %
or studies of the Z mixing with the photon in Drell-Yan processes at higher energies~\cite{Tanabashi:2018oca}.
...
In the EW Lagrangian of \refeq{eq:ew_lagrangian}, the Higgs $H$ is a complex scalar field and an $\text{SU(2)}_L$ doublet %
\begin{equation}
	\label{eq:higgs}
	H(x) = \mqty( H^+(x) \\ H^0(x) ) \ ,
\end{equation}
...
\end{equation}
the potential $V(H)$ finds its minimum for $H^\dagger H = v^2 / 2$ which %
...
To study what happens around the vacuum state, the scalar field can expanded around its vev %
and by choosing the unitary gauge three of the four real scalar fields %
are rotated away since they are unphysical, simplifying to
\begin{equation}
	\label{eq:vev_higgs}
	H (x) = \frac{1}{\sqrt{2}} \mqty( 0 \\ v + h(x) )\ .
\end{equation}
Using the definition of the EW fields in \refeq{eq:fields}, %
the covariant derivative of \refeq{eq:covariant} applied to the Higgs field reads 
\begin{equation}
	\label{eq:d_higgs}
...
\end{equation}
The Lagrangian terms with the Higgs field therefore become
\begin{align}
	\label{eq:all_higgs}
...
There is no principle by which the Yukawa coupling matrices $Y^\ferm$ should be \emph{a priori} diagonal, %
...
\end{equation}
with $\alpha$ the family generation index.
...
Looking at the lepton sector, the transformations of \refeq{eq:lepton_eigenmass} are not analogously defined for the neutrino fields.
...
It follows that after the spontaneous symmetry breaking caused by the nonzero Higgs vev the neutrinos %
...
\begin{equation}
	\bs{\nu}_R = \mqty(\nu_{eR} \\ \nu_{\mu R} \\ \nu_{\tau R})\ .
\end{equation}
These fields are chargeless under all the SM gauge groups and therefore are not directly coupled to any vector boson %
and as such their existence is not easily testable.
...
\begin{equation}
	\mathcal{L}_\text{EW} \supset - \qty( \cj{\bs{L}}_L\, Y^\ell H \,\bs{\ell}_R 
		      	     + \cj{\bs{L}}_L\, Y^\nu \widetilde{H}\, \bs{\nu}_R \ +\ \text{h.c.}) \ ,
\end{equation}
and using the expansion of \refeq{eq:vev_higgs} the Yukawa matrix can be diagonalised by a biunitary transformation %
...
\end{equation}
where the unitary matrix $U = V^{\nu\dagger}_L V^\ell_L$ is completely analogous to the CKM matrix of the quark weak charged current.
...
\end{align}
The GIM mechanism is still valid and no mixing takes place in neutral current interactions.
...
Approximating the neutrino fields to plane-waves, the flavour states are described by the following combination of mass states
\begin{equation}
...
The massive neutrino states $\ket{\nu_i}$ are eigenstates of the Hamiltonian, with the particle energies as eigenvalues:
\begin{equation}
	\label{eq:hamilton}
	\mathcal{H} \ket{\nu_i} = E_i \ket{\nu_i} = \sqrt{\vb{p}^2 + m_i^2} \ket{\nu_i}\ ,
\end{equation}
and $\vb{p}$ is the momentum of the produced flavour neutrino.
...
\end{equation}
The energies of ultrarelativistic neutrinos can be approximated by
\begin{equation}
	E_i \simeq E + \frac{m_i^2}{2E}\ ,
\end{equation}
and the propagation time is naturally replaced by the propagation length, i.e.\ $t \simeq L$, %
since it is easier to determine experimentally.
...
while the other parameters\,---\,the PMNS matrix elements and the neutrino masses\,---\,are constants of nature.
The probabilities for initial $\nu_e$ and $\nu_\mu$ to be detected as $\nu_e$, $\nu_\mu$, or $\nu_\tau$
is shown on \reffig{fig:osc_behave} as function of $\flatfrac{L}{E}$.
%for an initial $\nu_e$ and $\nu_\mu$.
...
The oscillating term is the result of the interference between different massive neutrinos %
which propagate at different velocities while coherency between states is preserved.
It could happen that neutrinos are produced or detected incoherently, for which interference terms %
do not appear, or that the detector resolution on propagation length or neutrino energy is limited %
and so the probability is averaged.
...
\end{equation}
and for $\alpha \neq \beta$ it can be shown that the maximum value this averaged probability %
can take is 
\begin{equation}
...
Apart from this limit scenario, the PMNS matrix and analogously the CKM matrix are generic %
...
Due to the unitarity of the neutral currents, only the weak charged current manifests these phases, %
...
It is typically parameterised as
%\vspace{-0.2em}
\begin{equation}
	\label{eq:pmns}
...
However, due to the structure of the oscillation probability (see \refeq{eq:oscillation_probability}) %
...
If the atmospheric mass difference is positive, then the order between neutrino masses is $m_1 < m_2 \ll m_3$, %
...
\end{equation}
where $E_\nu$ is the neutrino energy and $G_F$ is the Fermi constant introduced later in \refeq{eq:fermi_const}.
...
\end{equation}
After averaging over the helicities of the fermions, terms proportional to the neutrino currents are obtained
\begin{align}
...
\begin{equation}
	\mathcal{H}\ \phi_{\alpha \beta} (t) = %
		...
		\phi_{\alpha \rho} (t)\ .
\end{equation}
With this relation, the propagation of neutrinos in matter can be derived in complete analogy to the vacuum oscillation.
The evolution of neutrino states in time comes from solving the Schr{\"o}dinger's equation and %
the probability of flavour transition is then accordingly derived.
...
\end{equation}
The effective mixing angle $\theta_M$ is defined by
\begin{equation}
...
\end{equation}
By transforming the transition probability to the mass basis by means of $O_M$, %
...
From \refeq{eq:effective_angle}, it reads
\begin{equation}
...
to the vacuum or the detector density, which is practically negligible.
...
By comparing this energy to the measured mass differences from oscillation experiments (see \reftab{tab:nufit}) %
one can infer that at least two mass states of relic neutrinos are nonrelativistic.
However, neutrinos at these energies are almost impossible to detect with the current technologies and it will be a challenge %
...
Due to the fact that the density of \refeq{eq:nonrel_density} can never be greater than %
...
The processes of the $pp$ chain and CNO cycle releasing neutrinos are listed in \reftab{tab:pp_cno}.
...
For this reason, the SSM is considered to provide a credible estimate of the flux of neutrinos produced in the Sun, %
the distribution of which is reported in \reffig{fig:solar_nu_flux}.
...
as demonstrated in \reffig{fig:borex}.
...
It was the only SN explosion which was detected also through its neutrino emission.
In spite of this fact, supernova explosions are the most intense sources of neutrinos in the Universe.
Historically, SNe are classified by their spectral characteristics near maximal luminosity.
...
type II and type I.
Further classification of type I SNe comes from the presence of silicon and helium.
...
Old stars with a mass between \mbox{$8\,M_\odot \lesssim M \lesssim 40\,M_\odot$} tend to stratify %
in layers of elements undergoing fusion, with the lightest (H) in the outer shell burning progressively into heavier nuclei %
...
The \emph{pre-supernova} neutrinos from Si fusion, in fact,  make up roughly 1\,\% of the total energy that is emitted during the star's core-collapse.
...
The size of this region, known as \emph{neutrinosphere}, depends on the $\nu$ energies and flavours %
...
region of the SN, and their values strongly depend on the model (see for example \refrefs{Totani:1997vj, Nakazato:2012qf, Tamborra:2014hga}).
...
All core-collapse SNe in the causally-connected Universe are isotropically distributed %
...
because the study of SNe cannot rely just on very rare galactic core-collapse explosions.
...
Their interactions with nuclei in the atmosphere produce pseudoscalar mesons such as pions and kaons, %
which rapidly decay into charged leptons and neutrinos.
Due to helicity suppression, two-body decays of $\pi^\pm$ and $K^\pm$ favour the muon channel
\begin{align}
...
The energy range of primary cosmic rays goes from a few hundreds of MeV up to about \np{e20}\,eV, as it is shown in \reffig{fig:CR_flux}.
...
the interactions between cosmic rays and photons of the cosmic microwave background, known as %
...
It was observed by the Super-Kamiokande experiment that the flavour ratio of atmospheric neutrinos from %
positive and negative zenith angle were different, implying the presence of neutrino oscillation~\cite{Fukuda:1998mi}.
A correct prediction of the atmospheric flux becomes instrumental when studying oscillation physics; %
...
The predictions of the model used by the Super-Kamiokande experiment is shown in \reffig{fig:honda_flux}, %
thanks to which the evidence of neutrino oscillation was confirmed.
...
but also heavier mesons the amount of which depends on the energy of the protons and the choice of the target.
All these secondary particles decay leptonically or semileptonically via CC weak interactions thus creating the neutrino beam.
...
By changing the direction of the pulsed current it is possible to select and focus particles of the desired charge.
...
Despite the optimal design of the horn, kaons and other short-lived mesons cannot be deflected as efficiently %
...
%This effect is due to the forward boosting of the neutrinos in the laboratory frame: %
%in the rest frame neutrinos from two-body decay of pseudoscalar mesons are monoenergetic
...
In \reffig{fig:t2k_flux}, the T2K flux prediction is shown at different angles; %
...
\begin{align}
	...
	i \mathcal{M}_\text{NC} &= i\, \frac{g^2}{8\cos\vartheta}\ \cj{u}_{\nu_\alpha} \gamma^\mu (1-\gamma^5)\, u_{\nu_\alpha}
						    \,\frac{\eta_{\mu\nu}}{q^2 - m_Z^2+i\varepsilon}
						    \,\cj{u}_{\ferm} \gamma^\nu (g^\ferm_V-g^\ferm_A\gamma^5) u_{\ferm}\ ,
\end{align}
where $k$ is the momentum of the propagator and $V_{12}$ is a possible mixing between generic fermions $\ferm_1$ and $\ferm_2$.
Due to the typical energies involved, the momentum propagating between the particles is small %
...
\end{equation}
The constant $G_F = \np{1.1663787e-5}\,\text{GeV}^{-2}$ in \refeq{eq:fermi_const} %
is called Fermi's constant and it is obtain from measuring the muon lifetime~\cite{Tanabashi:2018oca}.
%the interactions %
...
This is depicted in \reffig{fig:cevns} where the CE$\nu$NS cross-section for a few different nuclear species %
is shown as a function of neutrino energy.
...
\begin{equation}
	\dv{\sigma}{T} \simeq \frac{G_F^2}{8\pi} M F^2(q^2) \qty[N + (4\sin^2\vartheta_\text{W} - 1)Z]^2 %
				\qty(2 - \frac{MT}{E^2})\ .
\end{equation}
The angular dependence of the scattering is then
\begin{equation}
...
with just a little contribution from the squared atomic number $Z$.
...
observed the coherent scattering for the first time using a \np{14.57}\,kg %
CsI[Na] scintillating crystal and collecting \np{1.76e23} protons on target~\cite{Akimov:2017ade}.
...
%	\quad (\alpha = \mu,\tau)\,.
%\end{align}
Using the effective four-point amplitudes of \refeq{eq:fermi_nc}, %
...
they are expressed in terms of the vector and axial coupling of \refeq{eq:gv_ga} and they read
\begin{align}
...
\end{align}
The quantity $q^2$ is the transfer momentum between the initial and final electrons. 
...
\end{equation}
which has a maximum value when the neutrino is not deviated in the scattering, i.e.\ $\cos\theta=1$,
and it equates
\begin{equation}
	T^\text{max}_e = \frac{2\,E_\nu^2}{m_e + 2 E_\nu}\ .
\end{equation}
The differential cross-section in \refeq{eq:nu_elastic_xsec} can be given as a function of the %
...
for the SNO and SK experiments~\cite{Fukuda:2002pe} in solving the solar neutrino problem (see \refsec{sec:nu_sun_sn}), %
since the rate of neutrino--electron scattering is related to the neutrino flavour.
...
For $q^2 \lesssim m_N$, elastic interactions dominate and are brought about by neutral currents.
...
\end{equation}
Once neutrinos acquire sufficient energy they can also undergo the analogous charged current counterpart, %
called ``quasi-elastic'' (CCQE) interaction, due to the fact that the recoiling nucleon changes its charge and mass transfer occurs.
...
particularly the \emph{axial mass} $m_A$ which is arbitrarily assumed to be $m_A = 1.0$\,GeV in many %
neutrino event generators.
%$G_A(q^2)$, and $G_P(q^2)$ 
The NCE cross-section has the same structure as in \refeqs{eq:cc_xsec_q}{eq:cc_xsec_t}, %
...
The neutrino--hadron cross-sections as a function of energy are shown in \reffig{fig:xsec}.
...
or the nucleon itself can excite into a baryonic resonance before decaying and emitting a charged or a neutral pion.
