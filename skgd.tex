...
Other searches, instead, suffer from irreducible background events, %
as for example proton decay and supernova relic neutrinos (SRN) analyses, %
and could avail of improved background suppression techniques.
Among these background sources, flasher events\footnote{A flasher event refers to the phenomena %
	where a PMT may misfire and result in many repetitive internal discharges, of both itself and nearby PMTs.
	Flasher events tend to have a broader hit timing distribution than neutrino events} %
and cormic ray muons are of particular importance for proton decay, %
whereas SRN searches are affected by atmospheric neutrinos, which dominate the region of interest %
$14\,\text{MeV} < E_\nu < 30\,\text{MeV}$, and muon-induced spallation.
These hindrances can be overcome by studying the yield of neutrons in neutrino interactions, such as the %
...
An important limitation comes from insufficient neutron tagging.
