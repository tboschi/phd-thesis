...
The Am-Be source is placed at the centre of a cube of bismuth germanite (BGO) crystal scintillator and %
...
\begin{figure}
	\begin{minipage}[t]{0.45\textwidth}
		\centering
		\resizebox{\textwidth}{!}{\input{source_spectrum.tex}}
		\caption{Normalised energy distribution of neutrons emitted by an AmBe source~\cite{PMID:4744412} %
			and neutrons and photons emitted by spontaneous fission of a %
...
called \emph{Time Series Analysis} method that uses multiplicity and time intervals between the detected events %
...
Differently from an Am-Be source the activity of which must be known precisely, with a californium %
source the neutron tagging calibration can be done in principle regardless of that information.
...
The source is placed in a simple device, shown in \reffig{fig:setup}, consisting of a cylindrical plastic scintillator %
(EJ-200), coated with mylar to contain optical photons, and optically mounted on a ET Enterprise 9902B series~PMT.
...
Without the source inside the scintillator, an event rate of 1.633\,Hz is measured, %
...
We also performed a GEANT4~\cite{Agostinelli:2002hh} simulation of the setup, with the aim of optimising the calibration device.
An ideal calibration device would absorb all the photons converting them into visible light without affecting the neutrons.
...
Different volumes and materials are tested for the scintillator in the simulation: BGO crystal and a generic vinyltouluene as scintillating materials, %
and a cube and cylinder with equal diameter and height are chosen.
The size is varied from 1\,cm to 40\,cm.
...
\begin{figure}
	\centering
	\resizebox{0.9\textwidth}{!}{\input{n_vs_gamma_2.tex}}
