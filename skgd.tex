...
\chapter{The future of Super-Kamiokande}
\label{cha:skgd}

The typical total cross-section for an accelerator neutrino ($E_\nu \sim1$\,GeV) %
is of the order of \np{e-38}\,cm\tapi{2} and for a solar neutrino ($E_\nu \lesssim10$\,MeV) %
it is roughly one order of magnitude lower, as it can be evinced from \reffigs{fig:cevns}{fig:xsec}.
...
candidate samples of CC quasi-elastic events (CCQE, see \refsec{sec:ccqe}) %
...
and the theory of neutron thermalisation and neutron capture in \refsec{sec:neutron}.
The Super-Kamiokande experiment is briefly introduced in \refsec{sec:sk}, with %
an emphasis on the techniques currently in place to control backgrounds, %
searches limited by irreducible background, and the improvements provisioned by the gadolinium phase of SK.
The neutron calibration procedure of the detector is explained in \refsec{sec:neutron_cal} %
...
using UV spectroscopy is presented in \refsec{sec:gad} and the first laboratory results are reported.
...
but microchannel plates (MCP) and silicon photomultipliers (SiPM) are used too (see e.g.\ \cite{Ambrosio:2016ijk}).
...
with some limited resolution (see \refeq{eq:e_reco}).
...
a precise particle identification is possible in some cases, such as in SK which is discussed in \refsec{sec:sk}.
...
in the centre of mass frame~\cite{Bickel}
\begin{equation}
	\label{eq:outgoing_E}
...
energy decrease in log-scale which on average equals to~\cite{Bickel}
\begin{equation}
...
their respective elastic cross-sections~\cite{Bickel}
\begin{equation}
...
by a water purification system (see \refsec{sec:gadolinium}).
...
Super-Kamiokande has provided %and other large scale water Cherenkov detectors have provided %
...
corresponds to inverse beta decay (IBD, see \reffig{sec:ccqe}).
...
Gadolinium-157 has the highest thermal neutron capture cross-section amongst any stable nuclides.
...
The efficiency dependency on the concentration is shown in \reffig{fig:gd_conc}.
...
From the considerations of \refsec{sec:neutron}, water is a good moderator for thermalising fast neutrons.
...
The candidates are selected by an analysis similar to the simulation of \refref{Irvine:2014hja} and outlined above.
...
Another possibility as neutron calibration source would be californium-252 which %
...
Being the average energy of a SF neutron from \tapi{252}Cf lower than the one from Am-Be, %
...
Some preliminary studies were performed to develop a neutron calibration device %
with californium for a generic water Cherenkov detector.
During laboratory measurements, a \tapi{252}Cf source with an activity of 4.3\,kBq at the time of its production was used.
...
\begin{figure}
	\centering
	\resizebox{\textwidth}{!}{\input{pics/n_vs_gamma_2.tex}}
...
\begin{figure}
	...
	\resizebox{0.49\linewidth}{!}{\input{pics/hdxabs.tex}}
...
\begin{figure}
	\centering
	\resizebox{0.6\linewidth}{!}{\input{pics/abs_example.tex}}
...
\begin{figure}
	\centering
	\resizebox{0.8\textwidth}{!}{\input{pics/gad_10cm.tex}}
...
Following \refeq{eq:linear_bl}, a linear interpolation is performed between the absorption peak differences, %
...
\end{equation}
The result of the fit is shown in \reffig{fig:gad_10cm}, from which a relative error of $\sim$3\,\% %
is estimated on the measurement of the 0.2\,\% concentration.
According to \reffig{fig:gd_conc}, this uncertainty translates to a $\sim$1\,\% error on the neutron capture efficiency.
...
In this chapter, the upcoming Super-Kamiokande gadolinium phase was outlined, %
focusing on its advantages and main challenges.
All of the current analysis will benefit from the the improved neutron tagging efficiency, %
up to 90\,\% with a Gd concentration of 0.2\,\%.
This is thanks to the smoking-gun signature of the neutron capture on gadolinium: %
a handful of photons with a total energy of about 8\,MeV and a typical thermalisation time of $\sim$30\textmu s.
Up until now, the sensitivity of SK to neutron tagging has been assessed with a calibrating device %
consisting of an Am-Be source encased in a BGO crystal cube.
Californium-252 has been investigated as an alternative neutron source, %
having a higher neutron activity and emitting multiple neutrons per fission events.
With the help of a GEANT4 simulation, it was found that the same calibration device used in SK %
is already of the optimal shape and material to be used with \tapi{252}Cf instead.
The Am-Be neutron source could then be replaced without further modifications of the device, %
bringing about a more precise neutron calibration.
One of the challenges of a Gd-loaded Cherenkov detector %
is to monitor the gadolinium concentration in water, as this can vary in time or be affected by the water flow and temperature.
A new method involving UV absorption spectroscopy is being developed with promising results %
which takes advantage of the strong Gadolinium absorption lines in the region between 270\,nm and 275\,nm.
The Beer-Lambert law relates absorbance to gadolinium concentration and so the latter can be estimate by %
measuring the intensity of UV light through a water sample.
The precision reached with this method on a 100\,cm water sample is around $\sim$0.3\,\% %
on the full load concentration of 0.2\,\%.
Furthermore, this technique allows for an almost continuous monitoring of the concentration, %
compared to the current atomic spectroscopy technique in place which is executed with a monthly frequency.
