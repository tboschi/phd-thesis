...
\end{equation}
The maximum angle is reached by ultra-relativistic particles with $\beta \simeq 1$.
...
but micro-channel plates (MCP) and silicon photomultipliers (SiPM) are used too (see for e.g.\ \ref{Ambrosio:2016ijk}).

This technique is largely used in neutrino detection, since it easily allow %
...
If the outgoing lepton has energy above the threshold of \refeq{eq:cherenkov_threshold}, %
...
Cherenkov detectors are also capable of particle identification, and the particle identification capabilities %
...
Sheets of white tyvek maximise the propagation of photons inside the OD and helps the reconstruction of %
...
The level contamination reached is usually less than 3\,mBq/m\tapi{3}.
...
corresponds to inverse beta decay (IBD, see \refsec{sec:ccqe}).
%which is two orders of magnitude larger than $\nu_e$ elastic scattering.
This measurement is afflicted at low energies by
i.e.\ muons below Cherenkov threshold that decay into electrons (invisible muon decay), %
...
rejected with neutron tagging.	% more detail in background events
If neutrons could be detected efficiently, these backgrounds could be reduced significantly and thus, %
making the detection of the DSNB (see \refsec{sec:nu_sun_sn} possible for the first time.
...
For laboratory measurement, a \tapi{252}Cf source with an activity of 4.3\,kBq at the time of its production was used.
...
\begin{figure}
	...
	\begin{minipage}[t]{0.5\textwidth}
		...
		ultraviolet lamp (UV), total organic compound lamp (TOC), de-ionising system (DI), %
...
the detectors components and it should be non-toxic. %it may be easily put into existing detectors.
...
In the collection tank, dissolved air is removed by a membrane de-gasifier and resins are used %
...
An alternative method for monitoring gadolinium concentration is under study.
...
\begin{figure}
	\centering
	\resizebox{0.7\linewidth}{!}{\input{pics/abs_example.tex}}
