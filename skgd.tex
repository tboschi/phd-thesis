...
Large volumes are, however, prone to intense background too, either from cosmic rays or natural radioactivity.
Passive or active techniques to reduce such undesired events must be put into place.
...
An outer detector system provides a passive and active veto to the fiducial region of both experiment.
...
e.g.\ EGADS~\cite{Ikeda:2019pcm}, ANNIE~\cite{Back:2019aqi}, and RENO~\cite{Ahn:2010vy}, %
and future experiments are planning to use the same principle.
In the next phase of SK, hundred tons of gadolinium sulfated will be dissolved in water, making %
...
and that the speed of the particle is constant with respect to the period of emission $\tau = \frac{\lambda}{c}$,
...
However, a real medium is always dispersive and allowed frequencies are restricted to the region for which $n(\lambda) > \frac{1}{\beta}$.
...
but silicon photomultipliers (SiPM) can also be employed.
...
and their sides with fiber-reinforced plastic.
...
and their Cherenkov rings are also less defined than a muon ones.
...
The materials of the walls of the tank, and particulary the glass of the PMTs, contains radioactive material, %
...
The selection algorithm in most of the SK analyses simply discards any reconstructed event the vertex of which is less than 200\,cm away from the ID walls.
...
This prevents radon in the air to dissolve in the clean water.
Radon contamination is usually less than 3\,mBq/m\tapi{3}, compared to unpurified air that can peak %
...
The predictions provide with several limits but although SK has the best world limit, no SRN has been detected yet~\cite{Bays:2011si, Zhang:2013tua}.
The average energies of these neutrinos are around a few tens of MeV, at which the largest cross-section %
corresponds to inverse beta decay (see \refsec{sec:ccqe}).
...
At energies $E_\nu > 1$\,GeV, the number of CCQE interactions decreases and often additional neutrons ($2p2h$) or pions (baryon resonance) %
are also released in the scattering process.
...
these particles can travel quite some distance from they production point without being detected.
Neutrons interact with nuclei in various way, depending on their energy and the target nuclues.
The interactions are divided in three typologies: scattering, which can be elastic or inelastic, %
...
\end{equation}
With this expression, we can compute the average number of collisions a neutron undergoes when %
...
\begin{figure}
	...
	\caption{Neutron cross-sections on hydrogen (left) and gadolinium (right) as a function of energy from \refref{Brown:2018jhj}. %
	       ...
	       The inset shows a detail of the resonance region of the cross-section, between 10\,eV and 100\,eV.}
	\label{fig:n_xsec}
\end{figure}
...
due to the fact that knowning the location of the capture significantly reduces the background.
...
which would impacts the capture time in a water Cherenkov detector.
...
reverse osmosis, vacuum and membrane degasifiers, and anion-exchange resins.
...
When the experiment will be de-comissioned in the future, the gadolinium will be extracted from the water system %
to avoid disperding it in the environment.
...
which still uses atomic absorption lines of Gd, but in solution in water and more frequently.
...
of uniform concentration, as linearly dependent to the optical path length via a material-specific constant~\cite{Beer_1852}
\begin{equation}
...
We find that these are the deciding factors for our application of monitoring gadolinium concentration, %
...
\begin{figure}
	...
	\caption{Schematic of the device used for testing the 100\,cm water sample in laboratory.
		...
		We noticed during the test that the pump was leaving a considerable amount of microbubbles in the sample.
...
The resolution should scale with the absolute quantity of Gd, or the length in this case.
Thanks to the high light throughput of the Ocean HDX spectrometer, %
we successfully studied absorbance spectra using a 100\,cm water sample.
In order to deal with a larger water volume, we also developed an automated device which is remotely operated to circulate the water.
...
The same analysis of the 10\,cm water sample is performed on the 100\,cm measurement and the result is reported in \reffig{fig:gad_1m}.
The relative error on the measurement is improved tenfold compared to the 10\,cm test.
For a concentration of 0.2\,\% the error is estimated to ~0.3\,\%, whereas for a 0.02\,\% concentration the error is around 1.5\,\%.
The calibration procedure, however, needs further investagation since the concentration seems to follow a quadratic law %
with the absorbance.
...
\begin{figure}
	...
	\caption{The inverse linear fit (red) between concentration and absorption %
		is shown togwther with data (blue) on the top panel for the 100\,cm water sample.
